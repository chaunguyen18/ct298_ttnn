...
$row_up = mysqli_fetch_assoc($result_gv);

$sql_gv1 = "SELECT giao_vien.*, trung_tam.TT_ten 
           FROM giao_vien 
           INNER JOIN trung_tam ON giao_vien.TT_ID = trung_tam.TT_ID
           WHERE giao_vien.GV_ID = '$id'
          ";

$result_gv1 = $conn->query($sql_gv1);
$row_up1 = mysqli_fetch_assoc($result_gv1);
if (isset($_POST['add'])) {
    ...
    if (!filter_var($trungtamEmail, FILTER_VALIDATE_EMAIL)) {
        ...
        $email_check_query = "SELECT GV_ID FROM giao_vien WHERE GV_email = '$trungtamEmail' AND GV_ID != '$id' LIMIT 1";
    ...
    if (empty($errors)) {
        ...
        $sql = "UPDATE giao_vien SET TT_ID = '$trungtamlevel', GV_ten = '$trungtamName', GV_sdt = '$trungtamPhone', GV_email = '$trungtamEmail', 
                GV_namkn = '$trungtamLocation',
                GV_quoctich = '$trungtamLat', GV_chuyenmon = '$trungtamLon'
                WHERE GV_ID = '$id'";

        if (mysqli_query($conn, $sql)) {
            echo "<script>
                    alert('Cập nhật giáo viên $trungtamName thành công!');
            ...
            <form id="mainForm" method="POST" enctype="multipart/form-data">
                <div class="form-container d-flex justify-content-between gap-3">
                    <div class="form-column d-flex flex-column flex-fill">
                        ...
                        <input type="text" id="trungtamName" name="trungtamName" value="<?php echo $row_up1['GV_ten']; ?>" required>

                        <label for="trungtamPhone">Số điện thoại:</label>
                        <input type="text" id="trungtamPhone" name="trungtamPhone" value="<?php echo $row_up1['GV_sdt']; ?>" required>
                        <?php
                        ...
                        <input type="email" id="trungtamEmail" name="trungtamEmail" value="<?php echo $row_up1['GV_email']; ?>" required>
                        <?php
                        ...
                        <select id="trungtamLevel" name="trungtamLevel" required>
                            ...
                            <?php
                              while ($row = mysqli_fetch_assoc($category_result)) {                                 
                                      $selected = ($row['TT_ID'] == $row_up1['TT_ID']) ? 'selected' : ''; 
                    ...
                    <div class="form-column d-flex flex-column flex-fill">
                        ...
                        <input type="number" id="trungtamLocation" name="trungtamLocation" value="<?php echo $row_up1['GV_namkn']; ?>" required>

                        <label for="trungtamLat">Quốc tịch</label>
                        <input type="text" id="trungtamLat" name="trungtamLat" value="<?php echo $row_up1['GV_quoctich']; ?>" required>

                        <label for="trungtamLon">Chuyên môn</label>
                        <input type="text" id="trungtamLon" name="trungtamLon" value="<?php echo $row_up1['GV_chuyenmon']; ?>" required>
